...
$sql = "SELECT KørselNavn, KørselsTid, KørselsPris FROM kørsel";
    ...
    <table class="table table-striped">
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                echo "<tr>";
                echo "<td>" . ($row["KørselNavn"]) . "</td>";
                echo "<td>" . ($row["KørselsTid"]) . "</td>";
                echo "<td>" . ($row["KørselsPris"]) . " kr.</td>";
                echo "<td><input type='radio' name='packages' value='" . ($row["KørselNavn"]) . "' data-price='" . ($row["KørselsPris"]) . "' id='$uniqueId' class='form-check-input package-radio'>";
